...
function mtl_render_membership_page()
{
    ...
    if (isset($_POST['mtl_add_member']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_add_member_nonce']) && wp_verify_nonce($_POST['mtl_add_member_nonce'], 'mtl_add_member_action')) {
            ...
            if ($first_name === '' || $last_name === '' || $phone_number === '' || $address_line1 === '' || $city === '' || $state === '' || $zip_code === '') {
                ...
                $email_in_use = $wpdb->get_var($wpdb->prepare(
                        ...
                        if (! empty($missing_cols)) {
                            ...
                            while (($row = fgetcsv($handle)) !== false) {
                                ...
                                if ($row_first === '' || $row_last === '' || $row_phone === '' || $row_address1 === '' || $row_city === '' || $row_state === '' || $row_zip === '') {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Missing a required field (first_name, last_name, phone_number, address_line1, city, state and zip_code are all required).');
                                    continue;
                                }
                                if (! array_key_exists($row_state, mtl_get_state_options())) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Invalid state "' . $row_state . '" -- must be a valid 2-letter U.S. state/territory or Canadian province code, or "N/A".');
                                    continue;
                                }
                                if (! in_array($row_country, mtl_get_country_options(), true)) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Invalid country "' . $row_country . '" -- must exactly match one of the supported country names.');
                                    continue;
                                }
                                if ($row_email === '' || ! is_email($row_email)) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Missing or invalid email address.');
                                    continue;
                                }
                                if ($row_donated === null) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'has_donated_tools must be "Y" or "N" (or left blank).');
                                    continue;
                                }
                                if (isset($seen_emails[strtolower($row_email)])) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Duplicate email "' . $row_email . '" also appears earlier in this file.');
                                    continue;
                                }
                                ...
                                if ($email_in_use) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Email "' . $row_email . '" already belongs to an existing member.');
                                    continue;
                                }
                                ...
                                if (! $inserted) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Database error while adding this member.');
                                    continue;
                                }
        ...
        if (isset($_POST['mtl_edit_member_nonce']) && wp_verify_nonce($_POST['mtl_edit_member_nonce'], 'mtl_edit_member_action')) {
            ...
            if (! $error) {
                ...
                if ($updated === false) {
                    ...
                    $has_verification = (bool) $wpdb->get_var($wpdb->prepare(
    ...
    if (isset($_POST['mtl_member_mark_returned']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_member_loan_nonce']) && wp_verify_nonce($_POST['mtl_member_loan_nonce'], 'mtl_member_loan_action')) {
            ...
            $mr_done = $wpdb->query($wpdb->prepare(
    ...
    if (isset($_POST['mtl_member_extend_loan']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_member_loan_nonce']) && wp_verify_nonce($_POST['mtl_member_loan_nonce'], 'mtl_member_loan_action')) {
            ...
            if (! preg_match('/^\d{4}-\d{2}-\d{2}$/', $ext_due) || ! strtotime($ext_due)) {
                ...
                $ext_active = $wpdb->get_var($wpdb->prepare(
    ...
    if (isset($_POST['mtl_member_cancel_reservation']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_member_cancel_reservation_nonce']) && wp_verify_nonce($_POST['mtl_member_cancel_reservation_nonce'], 'mtl_member_cancel_reservation_action')) {
            ...
            $cr_done = $wpdb->query($wpdb->prepare(
    ...
    if (isset($_POST['mtl_member_start_loan']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_member_start_loan_nonce']) && wp_verify_nonce($_POST['mtl_member_start_loan_nonce'], 'mtl_member_start_loan_action')) {
            ...
            $sl_res = $wpdb->get_row($wpdb->prepare(
            ...
            if ($sl_due_error) {
                ...
                $sl_ahead_count = (int) $wpdb->get_var($wpdb->prepare(
                ...
                if ($sl_ahead_count > 0) {
                    ...
                    $sl_on_loan = $wpdb->get_var($wpdb->prepare(
...
?>
        ...
        <table class="wp-list-table widefat fixed striped table-view-list" id="mtl-members-table">
            <thead>
                <tr>
                    ...
                    <th>Address</th>
                    <?php // data-date-col marks this as a date column for the sort handler below,
            ...
            <tbody>
                <?php if ($members) : ?>
                    <?php foreach ($members as $member) :
                        ...
                        $edit_url      = add_query_arg(array('mtl_action' => 'edit', 'member_id' => $member->member_id), $base_url);
    ...
    <?php // ---- Shared Manage Loan modal (one per page; the loan is set by JS
    ...
    <?php // ---- Shared Manage Reservation modal (one per page; the
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            <?php // If a loan/reservation action above just succeeded, reopen that
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function openModal(li) {
                ...
                <?php // Visibility follows the "if and only if first in queue"
