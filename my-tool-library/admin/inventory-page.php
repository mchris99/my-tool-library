...
function mtl_render_inventory_page()
{
    ...
    if (isset($_POST['mtl_add_tool']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_add_tool_nonce']) && wp_verify_nonce($_POST['mtl_add_tool_nonce'], 'mtl_add_tool_action')) {
            ...
            if ($barcode === '') {
                ...
                $barcode_in_use = $wpdb->get_var($wpdb->prepare(
                ...
                if ($inserted) {
                    ...
                    foreach ($category_ids as $cid) {
                        if ($cid > 0) {
                            $wpdb->insert(
                                $tbl_cat_map,
                                array('tool_id' => $new_tool_id, 'category_id' => $cid),
                                array('%d', '%d')
                            );
                        }
                    }
                    ...
                    foreach ($tag_ids as $tid) {
                        if ($tid > 0) {
                            $wpdb->insert(
                                $tbl_tag_map,
                                array('tool_id' => $new_tool_id, 'tag_id' => $tid),
                                array('%d', '%d')
                            );
                        }
                    }
                    ...
                    if ($header_row === false) {
                        ...
                        if (! isset($columns['tool_name']) || ! isset($columns['barcode'])) {
                            ...
                            while (($row = fgetcsv($handle)) !== false) {
                                ...
                                if ($row_tool_name === '') {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Missing tool_name.');
                                    continue;
                                }
                                if ($row_barcode === '') {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Missing barcode.');
                                    continue;
                                }
                                if (isset($seen_barcodes[strtolower($row_barcode)])) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Duplicate barcode "' . $row_barcode . '" also appears earlier in this file.');
                                    continue;
                                }
                                ...
                                if ($barcode_in_use) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Barcode "' . $row_barcode . '" is already used by an existing tool.');
                                    continue;
                                }
                                ...
                                if (! $row_inserted) {
                                    $bulk_failed_rows[] = array('row' => $row_number, 'reason' => 'Database error while adding this row.');
                                    continue;
                                }
                                ...
                                foreach ($row_category_ids as $cid) {
                                    $wpdb->insert($tbl_cat_map, array('tool_id' => $row_tool_id, 'category_id' => $cid), array('%d', '%d'));
                                }
                                foreach ($row_tag_ids as $tid) {
                                    $wpdb->insert($tbl_tag_map, array('tool_id' => $row_tool_id, 'tag_id' => $tid), array('%d', '%d'));
                                }
        ...
        if (isset($_POST['mtl_edit_tool_nonce']) && wp_verify_nonce($_POST['mtl_edit_tool_nonce'], 'mtl_edit_tool_action')) {
            ...
            if (! $error) {
                ...
                if ($updated === false) {
                    ...
                    foreach ($category_ids as $cid) {
                        if ($cid > 0) {
                            $wpdb->insert($tbl_cat_map, array('tool_id' => $edit_tool_id, 'category_id' => $cid), array('%d', '%d'));
                        }
                    }
                    foreach ($tag_ids as $tid) {
                        if ($tid > 0) {
                            $wpdb->insert($tbl_tag_map, array('tool_id' => $edit_tool_id, 'tag_id' => $tid), array('%d', '%d'));
                        }
                    }
    ...
    if (isset($_POST['mtl_retire_tool']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_retire_tool_nonce']) && wp_verify_nonce($_POST['mtl_retire_tool_nonce'], 'mtl_retire_tool_action')) {
            ...
            $rt_done = $wpdb->query($wpdb->prepare(
            ...
            if ($rt_done) {
                ...
                $rt_cancelled = $wpdb->query($wpdb->prepare(
    ...
    if (isset($_POST['mtl_reactivate_tool']) && current_user_can('manage_options')) {
        if (isset($_POST['mtl_reactivate_tool_nonce']) && wp_verify_nonce($_POST['mtl_reactivate_tool_nonce'], 'mtl_reactivate_tool_action')) {
            ...
            $ra_done = $wpdb->query($wpdb->prepare(
...
?>
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 10px; margin-top: 40px; margin-bottom: 10px;">
        ...
        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
            ...
            <?php // Sorting also covers fields that live in the detail panel
        ...
        <table class="wp-list-table widefat fixed striped table-view-list" id="mtl-inventory-table">
            <thead>
                <tr>
                    ...
                    <?php // Description, components, value, depreciation, acquired date
            ...
            <tbody>
                <?php if ($inventory) : ?>
                    <?php foreach ($inventory as $item) :
                        $edit_url = add_query_arg(array('mtl_action' => 'edit', 'tool_id' => $item->tool_id), $base_url);
    ...
    <?php // ---- Shared Quick Loan modal (one per page; the tool is set by JS
    ...
    <script>
        // ---- Quick Loan modal ----
        document.addEventListener('DOMContentLoaded', function() {
            <?php // JSON_HEX_TAG/AMP/APOS/QUOT so a member name/email containing
