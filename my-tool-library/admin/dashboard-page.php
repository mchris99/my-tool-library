...
{
    return array(
        'stat_members'  => array('title' => 'Current Membership',            'size' => 'small'),
        'stat_borrowed' => array('title' => 'Tools On Loan',                 'size' => 'small'),
        'stat_avg_age'  => array('title' => 'Average Tool Age',              'size' => 'small'),
        'overdue'       => array('title' => 'Overdue Tools',                 'size' => 'medium'),
        'reservations'  => array('title' => 'Upcoming Reservations',         'size' => 'medium'),
        'popular'       => array('title' => 'Most Popular Tools',            'size' => 'medium'),
        'unpopular'     => array('title' => 'Least Popular Tools',           'size' => 'medium'),
        'value_initial' => array('title' => 'Asset Value (Initial)',         'size' => 'small'),
        'value_current' => array('title' => 'Asset Value (Depreciated)',     'size' => 'small'),
        'depreciation'  => array('title' => 'Depreciation Overview',         'size' => 'small'),
        'areas'         => array('title' => 'Most Popular Member Areas',     'size' => 'medium'),
        'renters'       => array('title' => 'Member Rental Leaderboard',     'size' => 'medium'),
        'donors'        => array('title' => 'Donor Leaderboard',             'size' => 'medium'),
        'tool_history'  => array('title' => 'Tool History Lookup',           'size' => 'large'),
        'member_history' => array('title' => 'Member History Lookup',       'size' => 'large'),
    );
}
...
function mtl_get_dashboard_layout()
{
    ...
    foreach ($panels as $id => $def) {
        $layout[$id] = array('order' => $order++, 'size' => $def['size'], 'visible' => true);
    }
...
function mtl_render_dashboard_page()
{
    ...
    foreach ($inventory_rows as $row) {
        ...
        if ($oldest_tool === null || $age_years > $oldest_tool['age']) {
            $oldest_tool = array('name' => $row->tool_name, 'barcode' => $row->barcode, 'age' => $age_years);
        }
        if ($newest_tool === null || $age_years < $newest_tool['age']) {
            $newest_tool = array('name' => $row->tool_name, 'barcode' => $row->barcode, 'age' => $age_years);
        }
    }
    ...
    $most_popular  = array_slice(array_filter($popularity_rows, function ($r) {
        return (int) $r->loan_count > 0;
    }), 0, 8);
    $least_popular = array_slice(array_reverse($popularity_rows), 0, 8);
    $never_borrowed_count = count(array_filter($popularity_rows, function ($r) {
        return (int) $r->loan_count === 0;
    }));
    ...
    if ($th_tool_id > 0) {
        ...
        if ($th_tool) {
            ...
            $th_loans = $wpdb->get_results($wpdb->prepare(
    ...
    if ($mh_member_id > 0) {
        $mh_member = $wpdb->get_row($wpdb->prepare(
        ...
        if ($mh_member) {
            $mh_loans = $wpdb->get_results($wpdb->prepare(
            ...
            $mh_reservations = $wpdb->get_results($wpdb->prepare(
...
?>
    ...
    <?php
            ...
            ?>
                <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>">
                    <input type="hidden" name="page" value="mtl-dashboard">
                    <?php if ($date_from !== '') : ?><input type="hidden" name="mtl_from" value="<?php echo esc_attr($date_from); ?>"><?php endif; ?>
                    <?php if ($date_to !== '') : ?><input type="hidden" name="mtl_to" value="<?php echo esc_attr($date_to); ?>"><?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php // JSON_HEX_TAG/AMP/APOS/QUOT so a tool/member name containing
