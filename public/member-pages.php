...
function mtl_render_reservation_detail_panel($r, $self)
{
    ...
    <div class="mtl-res-detail-body">
        ...
        <a class="mtl-member-btn mtl-member-btn-danger" style="margin-top:16px;" href="<?php echo esc_url(add_query_arg(array('mtl_confirm' => 'one', 'rid' => (int) $r->reservation_id), $self)); ?>">Cancel this reservation</a>
    </div>
    <?php
    return ob_get_clean();
}
...
function mtl_render_member_reservations_page()
{
    ...
    $pickup_directions = trim((string) get_option(
    ...
    echo mtl_member_page_styles();
    ?>
    <?php // In the normal listing view the page is a two-column layout (list +
    ...
    <div class="mtl-member-wrap<?php echo $listing ? ' mtl-member-wrap-wide' : ''; ?>">
        ...
        <?php // Always shown (with an empty-state message when there's nothing
        ...
        <?php if ($confirm === 'one' && $confirm_row) : ?>
            <div class="mtl-member-card">
                ...
                <?php // "Are you sure?" for cancelling ONE reservation. Reached
        ...
        <?php else : ?>
            <?php // Two columns: the reservation list (left) and a detail box
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php // Plain link -> confirmation prompt (no delete yet). ?>
                                            <a class="mtl-member-btn mtl-member-btn-danger" style="padding:5px 12px; font-size:0.85em;" href="<?php echo esc_url(add_query_arg(array('mtl_confirm' => 'one', 'rid' => (int) $r->reservation_id), $self)); ?>">Cancel</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
function mtl_render_account_page()
{
    ...
    // The fallback text here matches setup-page.php's default exactly, so a
    // fresh install shows sensible copy before any admin has saved Setup.
    $verification_directions = trim((string) get_option(
    ...
    $tbl_res     = $wpdb->prefix . 'tool_reservations';
    $has_history = ! empty($loans) || (bool) $wpdb->get_var($wpdb->prepare(
    ...
    $has_active_reservation = (bool) $wpdb->get_var($wpdb->prepare(
    ...
    <div class="mtl-member-wrap">
        ...
        <?php if ($confirm_delete) : ?>
            <div class="mtl-member-card">
                ...
                <?php // "Are you sure?" for account deletion -- reached by the
